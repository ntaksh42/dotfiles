---
name: html-report
description: コードレビュー・調査・バグ解析・リサーチの結果を見やすいHTMLレポートとして保存・表示するスキル。「レポートを保存して」「結果をまとめて」「HTMLに出力して」「ドキュメントに残して」などと言われたときに必ず使用する。種別を自動判定し、設定された保存先へ番号付きHTMLを生成してブラウザで表示する。
---
...
`~/.claude/skills/html-report/output-config.toml` を読む。
...
2. HTMLファイルを書き込む
3. 保存したHTMLをデフォルトブラウザで開いて表示する（PowerShell: `Start-Process <ファイルパス>`）。HTMLをUI内に直接表示できるツールが利用可能な環境ではそちらを優先する
4. ユーザーに以下を報告する:

```
✅ 保存しました
ファイル: ~/reports/reviews/PR123.html
種別: コードレビュー
```
